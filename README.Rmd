...
First, create data objects from internal data files, which are later combined to the process state
...
Define the dependent variable, and create a cache, a necessary object used in the simulations.
...
Some diagnostics. Both indicate the extent to which the chain mixes (i.e., whether the thinning was chosen appropriately). For the autoCorrelationTest, lower values are better. Values above 0.5 are very problematic.
